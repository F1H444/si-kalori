    ...
    <>
      ...
      {!isMobileOpen && (
        <button 
          ...
          className="md:hidden fixed bottom-6 right-6 z-50 bg-yellow-400 border-4 border-black p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-y-1 transition-all"
        >
      ...
      {/* Kontainer Sidebar */}
      <aside className={`
        fixed md:sticky top-0 inset-y-0 left-0 z-40 w-72 bg-white border-r-4 border-black flex flex-col h-screen transition-transform duration-300 ease-in-out
      ...
      `}>
        ...
        <div className="p-6 border-b-4 border-black bg-black text-white flex justify-between items-center">
          ...
          <button onClick={() => setIsMobileOpen(false)} className="md:hidden text-white">
            <X className="w-6 h-6" />
          </button>
        </div>
